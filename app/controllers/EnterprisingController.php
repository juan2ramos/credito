...
class EnterprisingController extends Controller
{
    ...
    public function enterpricingSearchCreditList()
    {
        if (Auth::user()->roles_id == 4)
            return Redirect::to('/');
        ...
        if (Auth::user()->roles_id == 3) {
            ...
            $users = User::join('creditRequest', 'users.id', '=', 'creditRequest.user_id')->
            whereRaw('users.roles_id = 5 and creditRequest.state = 1 and users.location = ' . Auth::user()->location)
            ...
            $users = User::join('creditRequest', 'users.id', '=', 'creditRequest.user_id')->
            whereRaw('users.roles_id = 5 and creditRequest.state = 1')
                ->where(function ($query) use ($search) {
                    $query->where('name', 'like', '%' . $search . '%')
                        ->orWhere('last_name', 'like', '%' . $search . '%')
                        ->orWhere('email', 'like', '%' . $search . '%')
    ...
    public function enterpricingSearchSimpleList()
    {
        if (Auth::user()->roles_id == 4)
            return Redirect::to('/');
        ...
        if (Auth::user()->roles_id == 3){
            $users = User::
            whereRaw('users.roles_id = 5 and users.hasCredit = 0 and users.location = ' . Auth::user()->location . " and user_state < 2")
                ->where(function ($query) use ($search) {
                    $query->where('name', 'like', '%' . $search . '%')
                        ->orWhere('last_name', 'like', '%' . $search . '%')
                        ->orWhere('email', 'like', '%' . $search . '%')
                ...
                ->orderBy('user_state')->paginate(20);
        }
        else
        {
            $users = User::
            whereRaw('users.roles_id = 5 and users.hasCredit = 0 and user_state < 2')
                ->where(function ($query) use ($search) {
                    $query->where('name', 'like', '%' . $search . '%')
                        ->orWhere('last_name', 'like', '%' . $search . '%')
                        ->orWhere('email', 'like', '%' . $search . '%')
